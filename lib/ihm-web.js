...
// ---------------------------------------------------------------------
// Session-cookie source of truth — Supabase.ihm_session (singleton row)
// with a 5-minute in-memory cache per serverless cold start. Env var
// IHM_SESSION_COOKIE remains as a fallback for local dev / first-run.
// ---------------------------------------------------------------------
let _cookieCache = null;
let _cookieCachedAt = 0;
const COOKIE_TTL_MS = 5 * 60 * 1000;

function rowToCookieString(row) {
  if (!row) return null;
  const parts = [];
  if (row.cookie_ihm)            parts.push(`ihm=${row.cookie_ihm}`);
  if (row.cookie_aspnet_session) parts.push(`ASP.NET_SessionId=${row.cookie_aspnet_session}`);
  if (row.cookie_cf_clearance)   parts.push(`cf_clearance=${row.cookie_cf_clearance}`);
  if (row.cookie_email)          parts.push(`email=${row.cookie_email}`);
  if (row.cookie_extra)          parts.push(row.cookie_extra);  // already formatted "name=value; name=value"
  return parts.filter(Boolean).join('; ');
}

async function sessionCookie() {
  // Cache hit — avoid a Supabase read on every IHM call.
  if (_cookieCache && Date.now() - _cookieCachedAt < COOKIE_TTL_MS) return _cookieCache;

  try {
    const { data, error } = await supabase.from('ihm_session').select('*').eq('id', 1).maybeSingle();
    if (!error && data) {
      const str = rowToCookieString(data);
      if (str) {
        _cookieCache = str;
        _cookieCachedAt = Date.now();
        return str;
      }
    }
  } catch (_) {
    // Table might not exist yet (migration not applied) — fall through to env.
  }

  // Fallback: env var (first-run / migration pending).
  const envCookie = process.env.IHM_SESSION_COOKIE;
  if (!envCookie) {
    throw new Error('No IHM session cookies stored. Refresh via admin → IHM Session.');
  }
  _cookieCache = envCookie;
  _cookieCachedAt = Date.now();
  return envCookie;
}

// Call from /api/admin/ihm-session after saving new cookies so the NEXT
// IHM call reads fresh values instead of the stale cache.
...
async function ihmWebPost(path, params = {}, { accept = 'text/html, */*; q=0.01' } = {}) {
  ...
  const res = await fetch(IHM_BASE + path, {
    method: 'POST',
    headers: { ...(await baseHeaders()), 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8', Accept: accept },
    body: body.toString(),
  });
  if (!res.ok) {
...
async function ihmWebGet(path) {
  const res = await fetch(IHM_BASE + path, { headers: await baseHeaders() });
  if (!res.ok) {
    const text = await res.text().catch(() => '');
...
export async function createTerritory({ points, fileDate }) {
  ...
  const res = await fetch(IHM_BASE + '/Api/Territory', {
    method: 'POST',
    headers: { ...(await baseHeaders()), 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8', Accept: 'application/json, text/javascript, */*; q=0.01' },
