...
export async function runReceiver(req, res, processor) {
  ...
  try {
    ...
    // Soft-auth fallback: Lindy's bridge agent can't always set custom
    // headers on its outbound HTTP Request callbacks. We injected the
    // lindy_job_id on dispatch — only Lindy could know it. So if the
    // callback references a valid in-flight job, treat it as authentic.
    if (!sigOk && parsed?.lindy_job_id) {
      const { data: jobRow } = await supabase
        .from('lindy_jobs')
        .select('id, status, agent_name')
        .eq('id', parsed.lindy_job_id)
        .maybeSingle();
      if (jobRow && (jobRow.status === 'dispatched' || jobRow.status === 'callback_received')) {
        sigOk = true;
        sig.mode = 'job_id_match';
      }
    }
