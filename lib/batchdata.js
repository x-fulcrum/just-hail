...
export async function searchProperties(target, { take = 100, skip = 0 } = {}) {
  ...
  const total = json.status?.total ?? results.length;
  return {
...
function normalizeProperty(p) {
  ...
  return {
    ...
    owner_name:      [owner.firstName, owner.lastName].filter(Boolean).join(' ') || owner.fullName || null,
    year_built:      p.yearBuilt || p.year_built || null,
    estimated_value: p.valuation?.estimatedValue ?? p.estimatedValue ?? null,
    bedrooms:        p.bedroomCount ?? p.bedrooms ?? null,
    bathrooms:       p.bathroomCount ?? p.bathrooms ?? null,
    square_feet:     p.livingAreaSquareFeet ?? p.squareFeet ?? null,
    raw:             p,
  };
}
