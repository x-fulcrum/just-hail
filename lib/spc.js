...
// --------------------------------------------------------------
const BASE_OUTLOOK = 'https://www.spc.noaa.gov/products/outlook';
const BASE_EXPER   = 'https://www.spc.noaa.gov/products/exper';

export function spcOutlookUrl(day, kind = 'cat') {
  day = Number(day);
  if (day === 1 || day === 2) {
    if (kind === 'hail') return `${BASE_OUTLOOK}/day${day}otlk_hail.lyr.geojson`;
    return `${BASE_OUTLOOK}/day${day}otlk_cat.lyr.geojson`;
  }
  if (day === 3) {
    if (kind === 'prob') return `${BASE_OUTLOOK}/day3otlk_prob.lyr.geojson`;
    return `${BASE_OUTLOOK}/day3otlk_cat.lyr.geojson`;
  }
  // Day 4-8 live under /products/exper/day4-8/ (not under /products/outlook/).
  if (day >= 4 && day <= 8) {
    return `${BASE_EXPER}/day4-8/day${day}prob.lyr.geojson`;
  }
