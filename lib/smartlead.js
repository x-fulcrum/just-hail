...
// Set send-hours / cadence schedule. Smartlead REQUIRES this before
// startCampaign() will succeed — otherwise it errors with
// "Cron Exp value is empty! Please set the scheduler time and start
// the campaign!"
//
// schedule = {
//   timezone: 'America/Chicago',
//   days_of_the_week: [1,2,3,4,5],   // 0 = Sun, 6 = Sat
//   start_hour: '09:00',
//   end_hour: '17:00',
//   min_time_btw_emails: 20,         // minutes
//   max_new_leads_per_day: 45,       // soft per-campaign cap (mailbox cap still wins)
// }
export async function setCampaignSchedule(campaignId, schedule) {
  return request('POST', `/campaigns/${campaignId}/schedule`, {
    body: schedule,
  });
}
...
// Per-lead send/open/click/reply timestamps for a campaign.
// Used by the engagement-sync cron to reconcile our drip_touches with
// Smartlead's truth even when their webhooks don't fire reliably.
//
// Returns: { total_stats: number, data: [{ lead_email, sent_time,
//   open_time, click_time, reply_time, ... }] }
export async function getCampaignStatistics(campaignId, { offset = 0, limit = 500 } = {}) {
  return request('GET', `/campaigns/${campaignId}/statistics`, {
    query: { offset, limit },
  });
}
