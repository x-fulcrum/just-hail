...
async function dispatchVoicemail({ stateRow, step, lead, drip }) {
  // For now, voicemail dispatch goes through our Lindy specialized agent
  // (jh-voicemail-dropper). When we switch to Twilio direct, this changes.
  const phone = lead.mobile || lead.phone;
  if (!phone) {
    await logTouch({
      drip_campaign_id: drip.id, lead_id: lead.id, drip_lead_state_id: stateRow.id,
  ...
  // For now, log as a "queued" touch. Real implementation goes here when
  // we wire Twilio voice or Lindy voicemail-dropper webhook.
  await logTouch({
    drip_campaign_id: drip.id, lead_id: lead.id, drip_lead_state_id: stateRow.id,
    step_number: step.step_number, channel: 'voicemail',
    ...
    provider: 'pending',
    error_message: 'voicemail_dispatch_not_yet_wired',
  });
  return { sent: true, channel: 'voicemail', recipient: phone, note: 'logged but not sent — voicemail provider not yet wired' };
}

async function dispatchCall({ stateRow, step, lead, drip }) {
  // Same shape as voicemail for now — placeholder for when we wire Twilio voice.
  return await dispatchVoicemail({ stateRow, step, lead, drip });
}
...
async function getOrCreateSmartleadCampaign({ drip, sequence }) {
  ...
  // Push sequence (single step for now since each drip step IS a separate
  // dispatch — Smartlead just sends what we tell it to, when we tell it to)
  // Future: pre-load the whole sequence into Smartlead and use their cadence.
  // For MVP: each step we send is a one-off enrollment.
