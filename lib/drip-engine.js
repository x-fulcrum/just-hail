...
async function getOrCreateSmartleadCampaignForStep({ drip, step }) {
  ...
  // 5. Start the campaign — required for Smartlead to actually dispatch.
  //    A failure here is FATAL: leads added to a DRAFT campaign sit
  //    untouched forever, which is the bug we just hit. Throw so the
  //    calling dispatchEmail returns failed and retries on next tick.
  try {
  ...
  // 6. Persist the new campaign id keyed by step number
  await supabase
    .from('drip_campaigns')
    .update({
      metadata: {
