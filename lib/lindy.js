...
// ----------------------------------------------------------------
const AGENT_ENV = {
  // The "do anything" bridge agent — single delegation point. This is the
  // primary path for most tasks. The specialized agents below stay wired
  // for high-volume work (voicemail blasts, multi-call storm broadcasts).
  'jh-assistant':         { url: 'LINDY_ASSISTANT_URL',         token: 'LINDY_ASSISTANT_TOKEN'         },

  // Specialized agents — kept for jobs the bridge can't handle alone
  // (e.g., dialing 200 numbers in 10 minutes).
...
function callbackPathFor(agent) {
  switch (agent) {
    case 'jh-assistant':         return '/api/webhooks/lindy/assistant-result';
...
// ----------------------------------------------------------------
const RATE = {
  'jh-assistant':         { perMinute: 30 },   // bridge agent — moderate rate
...
// ----------------------------------------------------------------
// delegateToAssistant — fire a natural-language task to the bridge
// agent (jh-assistant). The bridge agent has access to all of Lindy's
// dynamic tool-loading + your existing Lindy connections (Gmail,
// Calendar, Twilio voice/SMS, web search, etc).
//
// task: string  — the natural-language instruction
// context:      — optional structured payload to give the agent
//                  more to work with (lead, campaign, storm, etc).
// thread_id:    — optional, ties messages into a single Lindy task
//                  thread (useful for embedded chat in admin).
// ----------------------------------------------------------------
export function delegateToAssistant({
  task,
  context = null,
  thread_id = null,
  lead_id = null,
  campaign_id = null,
  expect_response = true,   // false = fire-and-forget (no callback wait)
  ...rest
}) {
  if (!task || typeof task !== 'string') throw new Error('task (string) required');
  return dispatchAgent({
    agent: 'jh-assistant',
    lead_id,
    campaign_id,
    payload: {
      ...
      thread_id,
      expect_response,
      // The bridge agent's prompt instructs it to POST results back to
      // callback_url with these fields:
      //   { ok, summary, result, transcript?, error?, lindy_job_id }
    },
    ...rest,
  });
}
