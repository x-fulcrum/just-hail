...
// ----------------------------------------------------------------
// healthCheck — auth-only probe, doesn't burn scrape credits.
// We hit a deliberately invalid URL with a HEAD-style probe; any
// 200/400 response means the API key authenticated. We treat 402
// (insufficient credits) as "key valid, just out of quota" — still
// counts as configured + reachable.
// ----------------------------------------------------------------
export async function healthCheck() {
  ...
  try {
    // Hit /v1/scrape with a deliberately empty body — we want a 400
    // (validation error) which means the API key authenticated.
    // This consumes ZERO credits since the request never actually scrapes.
    const res = await fetch(`${BASE}/v1/scrape`, {
      method: 'POST',
      headers: {
        'authorization': `Bearer ${key()}`,
    ...
    if (res.status === 401 || res.status === 403) {
      return { ok: false, configured: true, latency_ms, error: 'auth_failed' };
    }
    if (res.status === 402) {
      // Out of credits but key works. Mark as warn, not down.
      return {
        ok: false, configured: true, latency_ms,
        error: 'insufficient_credits — key valid, add quota at firecrawl.dev/pricing',
        message: 'add credits',
      };
    }
    // 400 (validation error from empty body) or 200 = key works
